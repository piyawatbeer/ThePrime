...
<div id="app">
    ...
    <form id="saveForm" method="post" action="/service/pledgeslipsdel/{{$services->service_id}}" autocomplete="off"
                ...
                <div class="form-row">
                    <div class="form-group  col-sm-6">
                        ...
                        {{-- <label for="detailc"><b>ใบเสนอราคา :</b>
                            ...
                            data-name="ใบเสนอราคามัดจำ" role="button" class="btn btn-outline-danger btn-sm btn-delete" title="ลบ"><i
                                class="fa fa-trash"></i></a> --}}
                            {{-- <a href="/service/pledgeprint/{{$services->service_id}}" class="btn btn-outline-primary btn-sm"
                            title="พิมพ์"
                            target="_blank"><i class="fas fa-print"></i></a> --}}
                            {{-- @endif
        ...
        <div id="collapseTwo" style="min-height:100px;" class="collapse show" aria-labelledby="headingTwo" data-parent="#accordionExample">
            @if($design)
            <div class="row" style="padding:10px;">
                ...
                <div class="col-md-4">
                    <div class="card-header text-center align-self-center">
                        ...
                        <b>การออกแบบครั้งที่
                          <?php $numd=$numd+1; echo $numd;?>
                            {{-- <a href="#" data-href="{{ url('service/designaddf/'.$services->service_id.'') }}"
                            data-modal-name="ajaxModal" role="button" class="btn btn-outline-dark btn-create"
                            title="แก้ไข">
                            <i class="fas fa-edit"></i>
                            </a> --}}
                        @if($designs->users_id==Auth::user()->id)
                        <a href="" data-id="{{$designs->design_id}}" data-href="{{ url('service/designdel/'.$designs->design_id.'') }}" role="button"
                                class="btn btn-outline-danger btn-delete" title="ลบ">
                    ...
                    <div class="card-body text-center" style="min-height:80px">
                        <a href="{{URL::asset('/storage'.$designs->design_pic)}}" title="ดูรูปภาพ" target="_blank"><img src="{{URL::asset('/storage/'.$designs->design_pic)}}" class="img-thumbnail" alt="profile Pic" height="120px" width="120px"></a>
                        @if($designs->design_dropbox)
                    <br><a href="{{$designs->design_dropbox}}" style="color:blue;" title="ดูรูปภาพ" target="_blank">ดูรูป Dropbox <i class="fas fa-image   btn-outline-primary btn-sm"></i></a>
                        @endif
                    <br><font size="-2"><b>ผู้ออกแบบ:</b> {{$designs->name}}<br>
                        เมื่อ {{date('d-m-Y เวลา H:i น.', strtotime($designs->created_at))}}</font>
                    </div>
                    <div class="card-header text-center align-self-center">

                        <b>แสดงความคิดเห็น <a href="#"
                                data-href="{{ url('service/designmentf/'.$designs->design_id.'') }}"
                                data-modal-name="ajaxModal" role="button" class="btn btn-outline-success btn-create"
                                title="เพิ่ม">
                    ...
                    <div class="card-body align-self-start" style="min-height:80px">
                        ...
                        <?php $num=0;?>
                            ...
                            <hr>
                            <div class="text-right"><font  size="-2"><b>ผู้ตอบ:</b> {{$designlists->name}}<br>
                                เมื่อ {{date('d-m-Y เวลา H:i น.', strtotime($designlists->created_at))}}@if($designs->users_id==Auth::user()->id)
                                <a href="" data-id="{{$designlists->designlist_id}}" data-href="{{ url('service/designmentdel/'.$designlists->designlist_id.'') }}"
                                    role="button" class="btn btn-outline-danger btn-delete" title="ลบ">
                                    <i class="fas fa-trash"></i>
                                </a>
                                @endif</font>
                            </div>
            ...
            @else
            <h3 class="text-center"><font color="red" ><b><br>ไม่มีการออกแบบ</b></font></h3>
            @endif
        </div>
                </div>
</div>
...
<div class="form-row">
    <div class="form-group col-sm-12 text-center">
        <a href="{{ url('service/design/') }}"> <button type="button" class="btn btn-danger" data-dismiss="modal"><i
