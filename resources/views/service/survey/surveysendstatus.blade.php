...
@endforeach
<form id="saveForm" action="/service/surveysendstatusu/{{$services->service_id}}" method="post" autocomplete="off">
    {{ csrf_field() }}
    ...
    <div class="modal-body">
        ...
        <div class="form-group row text-right">
            <div class="col-sm-4">
                <label for="workname"><b>ผู้รับงานรังวัด:</b></label>
            </div>
            <div class="col-sm-8 text-left">
                @foreach ($user as $users)
                @if($services->surveyker==$users->id)
                {{$users->name}}
                @endif
                @endforeach
        ...
        <div class="form-group row text-right">
            <div class="col-sm-4">
                <label for="workname"><b>วันที่นัด:</b></label>
            </div>
            <div class="col-sm-8 text-left">
                {{date('d-m-Y', strtotime($services->meetdate))}}
            </div>
